...
class TableExtraction:
    ...
    def _assign_to_column(self, word, word_bb, columns, df, debug=False):
        ...
        for key, col_bb in columns.items():
            ...
            overlap = self._calculate_overlap(word_bb_temp, col_bb)

            if overlap > 10:
                if len(df[key]) > 0:
                    prev_obj = df[key][-1]
                    prev_overlap = self._calculate_overlap(
    ...
    def _calculate_overlap(self, bb1, bb2):
        ...
        overlap = intersection_area / float(bb2_area) * 100
        return overlap
    ...
    def detect(self, image_path: str):
        """Extract table data from an already cropped table image."""
        # Assume the entire image is a table, so we pass an empty dict for cords
        all_table_df = self._document_ocr.recognize(image_path, {})
        ...
        return table_data[0], None  # Return None for cords since we didn't detect any